...
fn main() {
    ...
    let unipass = generate_material(&mut stream, &U_PART_A, &U_PART_B, "U");
    let jupiter = generate_material(&mut stream, &J_PART_A, &J_PART_B, "J");
...
fn generate_material<const N: usize>(
    ...
    output.push_str("];\n");
    output.push_str(&format!(
        "pub(crate) fn {prefix_lower}_key() -> zeroize::Zeroizing<[u8; {N}]> {{\n    crate::secret::reconstruct(&{prefix}_FRAGMENTS, &{prefix}_ORDER, &{prefix}_ROTATE, &{prefix}_OFFSET)\n}}\n",
