...
- 本地 manifest 为商店版的下一补丁版，阻止浏览器用商店扩展覆盖；UniPass 网络请求默认发送随产物生成的 `runtime-config.json` 网络基线（当前 `5.3.2`），源码/CI 构建仍在线核验商店关系。商店版变更时必须同步网络基线，并把本地构建更新为其下一补丁版；用户可临时手动覆盖三段数字请求版号，且不影响发布门禁或本地 `5.3.3`。
- 设置页的“恢复默认”仅在已填写手动网络版本时可用；恢复后在固定 `1400ms` 内连续两击保存会解锁会话内“解锁高级模式”入口。进入高级模式后，本次 Popup/页面浮层生命周期内可查看账号：当前页显示“查看”按钮，全部应用的应用图标可进入账号列表；未进入时当前页不显示“查看”，应用图标保持原状且点击静默无操作。原保存按钮的隐藏固定 `1400ms` 三击自派生构建保持不变：目标本地 `x.y.z` 必须高于当前且 `patch >= 1`，每个版本段还必须在 Chrome 合法的 `0..65535` 范围内，网络版号严格派生为同主次版本的 `patch - 1`。它只复制当前静态 runtime、保留 `manifest.key` 和 WASM，不修改当前扩展，也不重新编译 Rust/WASM；缺少 `self-build-files.json` 时 fail closed。
...
- 最近客观验证：2026-09-11 `npm ci`、`npm run verify`、`npm run verify:wasm-reproducible` 通过，包含 Rust 1.98.1 QA、81 项 Node/WASM 测试、npm audit、类型检查和普通产物审计；本轮本机 branded Chrome 忽略 `--disable-extensions-except`，普通/二代 Chrome smoke 未出现 UniPass Service Worker，未标为通过；`npm run verify:hardened` 因本机缺少 Binaryen `wasm-opt` 在构建阶段 fail closed，未标为通过。CI 的 Binaryen 131 hardened job 与真实 UniPass/Jupiter 登录仍按 [Chrome 验收清单](docs/CHROME-ACCEPTANCE.md) 验收，未执行项不视为通过。
