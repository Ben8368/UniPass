...
- Normal/Advanced 都完整展示账号和备注；空密码账号不展示。Normal 允许使用账号密码 Fill，但 plaintext password 不返回 Popup/浮层；Advanced 通过 ephemeral capability 额外允许 Reveal、Credential panel 和 Copy Password。
...
- 最近客观验证：2026-09-11 `npm ci`、`npm run verify`、`npm run verify:wasm-reproducible`、`npm run verify:hardened`（含 hardened audit/diversity/Chrome smoke）通过；共 92 项 Node/WASM 测试。已安装并校验 Binaryen 131，用户级 `WASM_OPT` 指向 `wasm-opt.exe`；Chrome smoke 动态发现 Google Chrome，不绑定系统绝对路径，普通/derived/hardened smoke 通过。真实 UniPass/Jupiter 登录仍按验收清单人工确认。
