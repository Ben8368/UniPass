...
- 设置页保存按钮支持隐藏的 `1400ms` 三击自派生构建：目标本地 `x.y.z` 必须高于当前且 `patch >= 1`，网络版号严格派生为同主次版本的 `patch - 1`。它只复制当前静态 runtime、保留 `manifest.key` 和 WASM，不修改当前扩展，也不重新编译 Rust/WASM。
...
- 最近客观验证：2026-09-11 `npm run verify`、`npm run verify:wasm-reproducible` 通过，包含 Rust 1.98.1 QA、77 项 Node/WASM 测试、npm audit、类型检查和普通产物审计；Binaryen 131 下 hardened artifact/diversity 通过。完整 hardened/普通 Chrome smoke 在本机未出现扩展 Service Worker，未标为通过；真实 UniPass/Jupiter 登录仍按 [Chrome 验收清单](docs/CHROME-ACCEPTANCE.md) 人工复验，未执行项不视为通过。
