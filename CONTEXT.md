...
- 设置页保存按钮支持隐藏的固定 `1400ms` 三击自派生构建：目标本地 `x.y.z` 必须高于当前且 `patch >= 1`，每个版本段还必须在 Chrome 合法的 `0..65535` 范围内，网络版号严格派生为同主次版本的 `patch - 1`。它只复制当前静态 runtime、保留 `manifest.key` 和 WASM，不修改当前扩展，也不重新编译 Rust/WASM；缺少 `self-build-files.json` 时 fail closed。
