...
- WebDAV 以本地 AES-GCM 密文缓存为运行时副本、WebDAV 为同步/恢复后端；新设备由 endpoint、credential 与 Vault Key 建立缓存。连接材料受设备密钥保护；查看仍优先系统 WebAuthn、回退 4–32 位 PIN；ETag 冲突 fail closed。Chrome/Edge CSV 经用户选择、预览和 VaultCore 写入，不上传或持久化明文；详见 ADR 0004。
- 验证：2026-09-15 默认/hardened 门禁与 Chrome smoke 通过；Chrome 虚拟认证器覆盖系统 WebAuthn 注册、签名授权、重放拒绝与扩展 origin RP ID hash；154 项测试、依赖审计、类型检查、Rust QA、构建和产物审计通过。真实 WebDAV 断网恢复待验收。
