...
- 最近客观验证：2026-09-11 `npm ci`、`npm run verify`、`npm run verify:wasm-reproducible`、`npm run verify:hardened`（含 hardened audit/diversity/Chrome smoke）通过；共 92 项 Node/WASM 测试。已安装并校验 Binaryen 131，用户级 `WASM_OPT` 指向 `wasm-opt.exe`；Chrome smoke 动态发现 Google Chrome，不绑定系统绝对路径，普通/derived/hardened smoke 通过。2026-09-11 维护者已完成真实 UniPass/Jupiter 登录、Reveal/Unicode 密码、复制、60 秒清理、Popup/浮层 Fill 和 HTTPS/URL 拒绝验收，暂未发现问题；用户 scope 隔离、Service Worker/WASM 重启和 Self Derived Build 独立 Profile 仍未验收。
