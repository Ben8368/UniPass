...
- 每次门户 API 请求都会通过 Google Chrome 的官方扩展更新接口查询 UniPass 的最新 CRX 版本；不直接读取 Chrome Web Store 页面，因此不会触发该页面的 CORS 限制。查询或解析失败（5 秒超时）时安全回退到 `5.3.0`。默认不会保存版本号；点击 Popup 右上角齿轮才会打开“指定插件版本”，填写后手动覆盖，清空并保存即可删除覆盖并恢复自动获取。
