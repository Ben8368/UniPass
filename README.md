...
- 当前版本不监听 Cookie；若登录状态变化，重新点击扩展打开页面浮层即可刷新。HTTPS 页面使用 closed Shadow DOM 浮层；浏览器内部页、HTTP 和其他不可注入页面会打开扩展自身 Popup，且不会注入页面脚本。浮层打开时仅根据当前页面的渲染背景色与 `color-scheme` 自动选择浅色/暗色，不读取页面正文或页面存储；点击页面外部、按 Escape 或再次点击扩展会关闭浮层。
- UniPass 离线时可点击顶部“一键登录”。扩展会打开或复用登录页，依次点击“钛动科技”和固定 Tec-IAM 飞书授权页的“授权”；登录状态仍由 UniPass API 判断。账号选择、扫码、验证码、CAPTCHA 或授权内容变化时自动流程停止，需用户手动处理。
